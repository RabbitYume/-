<%@page import="dao.impl.SalesManagerImpl"%>
...
<%
	List<Sales> sales = new SalesManagerImpl().getSales();
	if (sales == null) {
		sales = new ArrayList<>();
	}
%>
						...
						<div class="">
							<div class="card">
								<div class="card-header">
									<h6 class="text-uppercase mb-0">销售记录</h6>
									<div align="right">
										<input class="btn btn-sm btn-primary" type="button" value="添加"
											onclick="window.location.href = 'goodsinsert.jsp'">
										<button class="btn btn-sm btn-success" onclick="exporExcel()">导出Excel</button>
									</div>
								</div>
								<div class="card-body">
									<form method="POST" id="form">
										<table class="table table-striped card-text" cellspacing="20"
											...
											<tbody>
												...
												%>
												<tr>
													<td><%=sale.getDanhao()%></td>
													<td><%=sale.getGs_id()%></td>
													<td><%=sale.getNum()%></td>
													<td><%=sale.getPrice()%></td>
													<td><%=sale.getSelldate()%></td>
													<td><%=sale.getPs_id()%></td>
													<td><a
														href="SalesSumServlet?id=<%=sale.getDanhao()%>&op=del"
														class="btn btn-sm btn-danger">删除</a></td>
													<td><a
														href="salesupdate.jsp?id=<%=sale.getDanhao()%>&op=update"
...
<script>
   	...
   	function exporExcel() {
    	window.open("ExcelServlet")//打开下载链接
    }
   </script>
</html>
