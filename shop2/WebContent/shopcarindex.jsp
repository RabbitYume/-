<%@page import="dao.impl.ShopCarManagerImpl"%>
...
>
<%
int pc_id = (int) session.getAttribute("pid");
List<ShopCar> shopcars=new ShopCarManagerImpl().getShopCars(pc_id);
if(shopcars==null){
						...
						<div class="">
							<div class="card">
								<div class="card-header">
									<h6 class="text-uppercase mb-0">购物车</h6>
									<div align="right">
										<input class="btn btn-danger" type="button" value="清空购物车"
											onclick="fun()" margin-top="10px">
									</div>
									</div>
								<div class="card-body">
									<form method="POST" id="form">
										<table class="table table-striped card-text" cellspacing="20"
											...
											<tbody>
												...
												%>
												<tr>
													<td><%=shopcar.getGc_id()%></td>
													<td><%=shopcar.getG_name()%></td>
													<td><%=shopcar.getNum()%></td>
													<td><%=shopcar.getPrice()%></td>
													<td><a
														href="ShopCarSumServlet?id=<%=shopcar.getGc_id()%>&op=del" class="btn btn-sm btn-danger">删除</a>
														<a
														href="shopcarupdate.jsp?id=<%=shopcar.getGc_id()%>&op=update" class="btn btn-sm btn-info">修改</a>
												</tr>
												<%
													}
												%>
									...
									<br />
									<div align="right">
										<input class="btn btn-success" type="button" value="结算" onclick="total()">
										<input class="btn btn-secondary" type="button" value="返回"
											onclick="window.location.href = 'zhuye.jsp'">
									</div>
								</div>
							</div>
						</div>
										...
										<div class="container-fluid">
											<div class="row">
												...
												<div
													...
													<p class="mb-0">Copyright &copy; 2019.Company name All
...
<script>
   	...
   	function fun(){
   		var r=confirm("确认清空吗？");
   		if(r){
   			location.href="ShopCarSumServlet?op=delall";
   		}
   	}
   	function total(){
   		var r = confirm("确认结算吗？");
   		if(r){
   			location.href="ShopCarSumServlet?op=total";
   		}
   	}
   </script>
</html>